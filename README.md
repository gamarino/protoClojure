...
`ctest` registers **235 test cases: 178 conformance fixtures, 56 unit tests** (lexer, reader, runtime map, value equality and hashing) **and 1 CLI check** (`--help`). All of them pass. The benchmark numbers above are reproduced by `./benchmarks/bench.sh`, the actor throughput numbers by `./benchmarks/actor-bench.sh`.
...
ctest --test-dir build_release -j1           # 235 cases: 178 fixtures + 56 unit tests + 1 CLI check
