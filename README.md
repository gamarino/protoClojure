...
`ctest` registers **259 test cases: 197 conformance fixtures, 61 unit tests** (lexer, reader, runtime map, value equality and hashing) **and 1 CLI check** (`--help`). All of them pass. The benchmark numbers above are reproduced by `./benchmarks/bench.sh`, the actor throughput numbers by `./benchmarks/actor-bench.sh`.
...
ctest --test-dir build_release -j1           # 259 cases: 197 fixtures + 61 unit tests + 1 CLI check
