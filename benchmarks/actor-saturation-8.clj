;; Saturation — 8 actors x 600 CPU-bound messages, one sender thread.
;;
;; The twin of protoScala's `benchmarks/actors/actor-saturation-8.scala`, so
;; that the actors-versus-workers curve can be read on the same machine with
;; the same shape. It also mirrors protoST's `benchmarks/actors/saturation_8a.st`.
;;
...
;; and the workers are the constraint. With 8 actors the single-method
;; invariant allows at most 8 to run at once.
;;
...
;; over the 8 actors is 4800 * 200,010,000. The closing probe returns the state
...
(def processed (run))
(println (str "mode=saturation-8 messages=" (+ TOTAL ACTORS)
              " processed=" processed))
(println (actor-stats))
